...
"#,
    ...
    cmd.assert()
        .success()
        .stderr(contains("omarchy-restart-hyprlock not found in PATH").not());
}
...
"#,
    ...
    let link_path = env
        .home
        .join(".config/hypr/themes/hyprlock/omarchy-default");
...
fn hyprlock_links_omarchy_default_from_config_omarchy_themes_dir() {
    ...
    fs::create_dir_all(&omarchy_default).unwrap();
    fs::write(
        omarchy_default.join("hyprlock.conf"),
        "omarchy-theme-config",
    )
    .unwrap();
...
fn hyprlock_warns_when_main_config_not_sourcing_current_theme() {
    ...
    cmd.assert().success().stderr(predicates::str::contains(
        "does not source current theme hyprlock config",
    ));
}
...
fn hyprlock_does_not_override_non_managed_host_config() {
    ...
    cmd.assert().success().stderr(
        contains("preserving custom")
            .and(contains("does not source current theme hyprlock config")),
    );
